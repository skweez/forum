...
directives.directive('markdown', function($filter) {
	/**
	 * Creates a markdown element from a href string. This element can be added
	 * to a markdown.js JsonML tree. If the href string points to an image, an
	 * img (<img> in html) element is created. Otherwise a link element (<a>
	 * in html) is created.
	 * 
	 * @param href
	 *            the href string
	...
	/**
	 * Search for hyper links in a markdown tree and convert them to richer
	 * objects like clickable links or inline images.
	 * 
	 * @param part
			...
			linkify(part, {
				callback : function(text, href) {
					if (href) {
						...
						tree.splice(index + 1, 0, markdownObject);
